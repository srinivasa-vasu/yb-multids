...
public class KVRetryPolicy extends ExceptionClassifierRetryPolicy {
	...
	private final Map<String, List<String>> specialCodes = Map.of("XX000", List.of("schema version mismatch", "duplicate request"));
...
//			.filter(state -> (state.matches(SQL_STATE) || (specialCodes.containsKey(exception.getSQLState()) && (
...
//			.isPresent();
	...
	private final Predicate<SQLException> sqlMsgPredicate = exception ->
			Optional.ofNullable(exception.getMessage()).filter(msg -> msg.toLowerCase().matches(SQL_MSG))
					.isPresent();
